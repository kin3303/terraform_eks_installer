...
#   https://developer.hashicorp.com/vault/docs/platform/k8s/helm/examples/injector-tls
###########################################################################################
...
injector:
  ...
  webhookAnnotations: {}
  certs:
    secretName: "${kubernetes_secret.vault_injector_tls_certificate.metadata[0].name}"
    caBundle: "${local.caBundle}"
...
resource "helm_release" "vault" {
  ...
  depends_on = [
    ...
    kubernetes_secret.vault_injector_tls_certificate
  ]
}
...
module "eks_private_cert_injector" {
  source            = "../terraform-aws-eks-private-tls"
  organization_name = "Plateer, Inc"
  common_name       = "idtplateer.com"
  ca_common_name    = "idtplateer.com"
  dns_names = [
    "vault-injector",
    "vault-injector.vault-server",
    "vault-injector.vault-server.svc",
    "vault-injector.vault-server.svc.cluster.local",
    "vault-0.vault-internal",
    "vault-1.vault-internal",
    "vault-2.vault-internal"
  ]
...
resource "kubernetes_secret" "vault_injector_tls_certificate" {
  metadata {
  ...
  data = {
    "vault-injector.crt" = "${module.eks_private_cert_injector.public_key}"
    "vault-injector.key" = "${module.eks_private_cert_injector.private_key}"
    "vault-injector.ca"  = "${module.eks_private_cert_injector.ca_public_key}"
  }
  depends_on = [
    module.eks_private_cert_injector
  ]
...
locals {
  caBundle = base64encode(module.eks_private_cert_injector.ca_public_key)
}