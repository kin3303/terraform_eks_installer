locals {
  ...
  resource_names = {
    # IAM
    efs_csi_iam_policy_name = replace(random_id.name.hex, "replaceme", "efs-csi-driver-policy")
    efs_csi_iam_role_name   = replace(random_id.name.hex, "replaceme", "efs-csi-driver-role")
  }
}
